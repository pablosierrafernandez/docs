...
Returned when a resource does not exist, or when the endpoint URL itself is incorrect.
...
The API returns `404` whenever a referenced resource (configuration, transcription, evaluator, or participant) does not exist in your account. Verify the ID is correct by listing the resource first:
